...
export default function MemberDashboard() {
  ...
  const handleScheduleDonation = async (event: FormEvent) => {
    ...
    try {
      // 1. Strip invisible quotes to prevent database lookup failures
      const cleanMtn = activeMtn?.replace(/['"]/g, '').trim().toUpperCase();
      ...
      const res = await fetch('/api/appointments/donate', {
        ...
        body: JSON.stringify({
          ...
          appointment_date: appointmentDate,
          collection_method: collectionMethod,
          confirm_no_new_risks: confirmNoNewRisks // 2. Send the checkbox state!
        })
      });
      const data = await res.json();
          ...
          <div className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-2 space-y-2 text-sm text-slate-700">
                    ...
                    <div><span className="font-semibold">Donor Tracking No:</span> {profile.dtn ?? 'N/A'}</div>
                    <div><span className="font-semibold">Receiver Tracking No:</span> {profile.rtn ?? 'N/A'}</div>
                  </div>
                </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              ...
              <div className="space-y-4">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between gap-4 mb-4 pl-3">
                    ...
                    <span className={`inline-flex items-center rounded-full px-3 py-1 text-[10px] font-bold uppercase ${profile.screening_valid ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-800'}`}>
                      {profile.screening_valid ? 'Valid' : 'Invalid'}
                    </span>
                  </div>
                ...
                {(!profile.dtn || profile.dtn_status?.toLowerCase() === 'completed' || profile.dtn_status?.toLowerCase() === 'rejected') && (
                  <>
                    <div className="mb-4">
                    ...
                    {donationMessage && (
                      <div className={`rounded-xl border px-3 py-2 text-sm mb-4 ${donationMessage.includes('Network') || donationMessage.includes('Unable') ? 'bg-red-50 border-red-200 text-red-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>
                        {donationMessage}
                      </div>
                    )}
                    ...
                    <form onSubmit={handleScheduleDonation} className="space-y-4 flex-1 flex flex-col">
                      <div className="grid gap-4 sm:grid-cols-2">
                        <label className="block text-sm text-slate-600">
                          ...
                          {/* REMOVED disabled={!profile.eligible_to_donate} */}
                          <input type="datetime-local" required value={appointmentDate} onChange={(e) => setAppointmentDate(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#E04A75]" />
                        </label>
                        <label className="block text-sm text-slate-600">
                          <span className="mb-1 block text-[10px] uppercase tracking-wider font-black text-slate-400">Collection Method</span>
                          <select value={collectionMethod} onChange={(e) => setCollectionMethod(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#E04A75]">
                      ...
                      <button type="submit" disabled={donationSubmitting} className="w-full rounded-xl bg-[#1A1A1A] px-4 py-3.5 text-xs font-black text-white hover:bg-slate-800 transition-colors disabled:opacity-50 uppercase tracking-wider mt-auto shadow-sm">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] uppercase font-black text-slate-400 tracking-wider">
                        <th className="py-3 px-6">Donation Ticket (DTN)</th>
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs text-slate-700 font-medium">
                      {history.map((bottle) => (
                        <tr key={bottle.collection_id} className="hover:bg-slate-50/30">
                          {/* Use DTN if available, fallback to BOT if it was a legacy walk-in without an appointment */}
                          <td className="py-4 px-6 font-mono font-bold text-slate-500">
                            {bottle.dtn_reference || `#BOT-${bottle.collection_id}`}
                          </td>
                          ...
                          <td className="py-4 px-6">
                            {/* Simplified success status! */}
                            <span className="text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded font-bold uppercase text-[9px]">
