          ...
          {quarantineQueue.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 text-[10px] uppercase font-black text-slate-400 tracking-wider bg-slate-50/50">
                    <th className="py-4 px-6">Batch ID</th>
                    <th className="py-4 px-6">Volume</th>
                    <th className="py-4 px-6">Temp (°C)</th>
                    <th className="py-4 px-6">Time (Min)</th>
                    <th className="py-4 px-6 text-right">Assay Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                  {quarantineQueue.map((batch) => {
                    ...
                    return (
                      <tr key={batch.batch_id} className={`hover:bg-slate-50/60 transition-colors ${isFlagged ? 'bg-red-50/30' : ''}`}>
                        ...
                        <td className="py-4 px-6 text-right flex justify-end gap-2">
                          <button disabled={qaProcessingId !== null || isFlagged} onClick={() => handleQaDecision(batch.batch_id, 'Passed')} className="bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-[10px] uppercase px-4 py-2 rounded-lg hover:bg-emerald-600 hover:text-white transition-all disabled:opacity-40 shadow-sm">✅ Pass</button>
                          <button disabled={qaProcessingId !== null} onClick={() => handleQaDecision(batch.batch_id, 'Failed')} className="bg-red-50 text-red-700 border border-red-200 font-bold text-[10px] uppercase px-4 py-2 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm">❌ Discard</button>
                        </td>
                      </tr>
                    );
                  })}
